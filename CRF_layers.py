...
class CRF(nn.Module):
    ...
    def forward(self, feats, logits):
        ...
        """
        feats_norm = torch.norm(feats, p=2, dim=2, keepdim=True) #按2维度求feats的2范数,就是把每个224*224的特征向量求绝对值，etc:sqrt(x1^2+...+xn^2)
        pairwise_norm = torch.bmm(feats_norm,
                                  torch.transpose(feats_norm, 1, 2))
        pairwise_dot = torch.bmm(feats, torch.transpose(feats, 1, 2)) #每个特征向量的点乘
